...
// Schema changes are deployment-time migrations, never request-time behavior.
router.post('/setup', (_req, res) => {
  res.status(410).json({
    success: false,
    message: 'This setup endpoint has been retired. Apply database migrations during deployment.',
  });
});
