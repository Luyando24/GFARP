...
export default function PlanManagement() {
  ...
  const fetchPlans = async () => {
    try {
      // Fetch Academy Plans
      const academyData = await Api.get<{ data: SubscriptionPlan[] }>('/subscriptions/plans?targetType=ACADEMY&includeInactive=true');
      setAcademyPlans(academyData.data);

      // Fetch Player Plans
      const playerData = await Api.get<{ data: SubscriptionPlan[] }>('/subscriptions/plans?targetType=INDIVIDUAL&includeInactive=true');
      setPlayerPlans(playerData.data);

      // Fetch Agency Plans
      const agencyData = await Api.get<{ data: SubscriptionPlan[] }>('/subscriptions/plans?targetType=AGENCY&includeInactive=true');
      setAgencyPlans(agencyData.data);
    } catch (error) {
